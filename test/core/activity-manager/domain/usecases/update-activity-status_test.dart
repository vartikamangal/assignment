...
class MockUpdateActivityStatusService extends Mock implements UpdateActivityStatusService {}

void main(){
  MockUpdateActivityStatusService service;
  UpdateActivityStatus useCase;
  ...
  group('USECASE : setTarget', () {
    test('should update the activityStatus with the help of service', () async {
      //arrange
      when(service.updateStatus(status: tStatus, actionId: tAActionId,))
          .thenAnswer((_) async => const Right(tActivityStatus));
      //act
      final result = await useCase(const UpdateActivityStatusParams(status: tStatus, actionId: tAActionId,));
      //assert
      verify(service.updateStatus(status: tStatus, actionId: tAActionId,));
