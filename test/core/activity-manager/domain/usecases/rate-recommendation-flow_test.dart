...
class MockRateRecommendationFlowService extends Mock implements RateRecommendationFlowService {}

void main(){
  MockRateRecommendationFlowService service;
  RateRecommendationFlow useCase;
  ...
  const tFeedbackModel=FeedbackModel(
      subjectMoodVO: FeedbackMoodModel(mood: "GOOD", activityType: "RECOMMENDATION"),
      minutesSpent: 4,
      feedbackThoughts: "",
    ...
    test('should rate the recommendationFlow with the help of service', () async {
      //arrange
      when(service.rateRecommendation(feedback: tFeedbackModel))
          .thenAnswer((_) async => const Right(unit));
      //act
      final result = await useCase(const RateRecommendationFlowParams(feedback: tFeedbackModel));
      //assert
